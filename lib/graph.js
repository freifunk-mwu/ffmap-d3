define("graph", [
  ...
  function reload(json) {
    ...
    var nNodes = data.nodes.filter(function(d) {
                   return d.system.role !== "gateway" && d.system.role !== "service" && d.system.role !== "backbone" && d.flags.online
                 }).length,
        nAllNodes = data.nodes.filter(function(d) {
                   return d.system.role !== "gateway" && d.system.role !== "service" && d.system.role !== "backbone"
                 }).length,
        nServices = data.nodes.filter(function(d) {
                   return d.system.role === "service" && d.flags.online
                 }).length,
        nBackbones = data.nodes.filter(function(d) {
                   return d.system.role === "backbone" && d.flags.online
                 }).length,
  ...
  function update() {
    ...
    node.attr("class", function(d) {
          var s = ["node"]
          if (d.system.role !== "gateway" && d.system.role !== "service")
            if (data.meta.gluon_release > d.firmware)
              s.push(["legacy"])
